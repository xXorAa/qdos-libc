 ...
 *              q f o r k v
 *
 *  QDOS comaptible routine to fork off another process specifying owner.
...
pid_t qforkv _LIB_F4_ ( const jobid_t,  owner, \
                        const char *,   name, \
                        int *,          chans, \
                        char * const *, argv)
{
    return (pid_t)_forkexec( name, 2, chans, argv, owner);
}
